...
- **电脑总时长（不区分应用）**：不管孩子用哪个软件，每天累计用满就锁屏。周内 / 周末分开限额，也可以给某几天自定义分钟数（5–600）。用时基于系统单调时钟（QueryUnbiasedInterruptTime），改系统时间骗不了它；**锁屏和睡眠的时段不扣时间**，孩子锁屏去吃饭不会白白烧额度；跨天自动重置。配合「开机后自动开始守护」，孩子一开机就开始计，不用家长手动点。
...
- **时间用完**：先给 2 分钟「保存进度」宽限——倒计时停在 0，书桌软件还能用来把存档和文档保存完，之后才清场锁屏；进入「时间用完」桌面只留系统本身，明天自动恢复，或输入家长密码暂停。最后 10 / 5 / 1 分钟，孩子界面会分级提醒，到点不搞突然袭击。
...
dotnet run --project tools\Sim    # 全功能仿真：52 项产品功能检查（假适配器，不碰真实进程/网络/注册表）
...
澄时的行为（锁屏、关闭名单外的程序、SYSTEM 服务常驻、密码门禁）与杀毒软件启发式盯防的「可疑行为」高度重合，而澄时是免费开源软件、没有购买代码签名证书，所以 Windows SmartScreen 或第三方杀软可能提示「未知发布者 / 已保护你的电脑」。这是误报：全部代码开源、所有数据只存本机。安装时点「仍要运行」即可；若杀软拦截守护服务，把 `%ProgramFiles%\Chengshi` 加入白名单。不放心可以直接从源码构建（见下）。
