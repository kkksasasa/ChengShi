...
三块核心界面按 `design/` 里的设计稿用 WPF/XAML 落地：

| 家长仪表盘 | 孩子书桌 · 申请加时 | 时间用完锁屏 |
|---|---|---|
| ![家长仪表盘](design/ui-01-parent-dashboard.png) | ![孩子书桌与申请加时](design/ui-02-kid-desk-extra-time.png) | ![时间用完锁屏](design/ui-03-time-up-lock.png) |

- **家长仪表盘**：书桌模板卡（写作业带「已断网」角标）、每日屏幕时间圆环（周内/周末两档）、最近七天用量柱状图、底部「开始守护」主操作。
- **孩子书桌**：守护中只留白名单应用磁贴与大倒计时；时间不够点「申请加时」，家长选时长、输密码当场批准。
- **时间用完**：切到夜晚锁屏场景，明天早上自动恢复；家长可加时或输密码解锁，角标常驻「澄时守护中 / 睡觉时段」。
